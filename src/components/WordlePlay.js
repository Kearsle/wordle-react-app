...
export default observer(function WordlePlay() {
  ...
  async function getWordlist() {
    const wordlistRes = await axios.get(
      `http://localhost:8000/wordle/wordlist/${params.wordlist}`
    );
    ...
    <div className="wordle-container">
      ...
      {store.loading === false && (
        <>
          ...
          {store.gameOver === true &&
            (store.won ? (
              <div className="wordle-game-over-cover">
                <div
                  className="wordle-game-over-card"
                  id="wordle-game-over-win"
                >
                  <h1 className="wordle-game-over">You win!</h1>
                </div>
              </div>
            ) : (
              <div className="wordle-game-over-cover">
                <div
                  className="wordle-game-over-card"
                  id="wordle-game-over-loss"
                >
                  <h1 className="wordle-game-over">You lost.</h1>
                </div>
              </div>
            ))}
        </>
